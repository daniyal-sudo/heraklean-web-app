  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        ...
        const response = await fetch('http://localhost:5001/api/auth/notification', {
          headers: {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:5001/api/auth/getinfo/${trainerId}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        ...
        if (data.success && data.profilePic) {
          ...
          const imageUrl = `http://localhost:5001/${fixedProfilePic}`;
