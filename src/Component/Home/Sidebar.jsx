...
const Sidebar = () => {
	...
	const navItems = [
		...
		{ name: "Diet Plans", image: "/fluent_channel-16-regular.png", path: "/dietplan" },
	...
	return (
		<>
			...
			<div className="text-center display_class">
				<img src="/image 45.png" alt="logo" className="img-fluid side_logo_height_width" />
			</div>

			{/* Sidebar Navigation */}
			<nav className={`custom-sidebar ${isOpen ? "open" : ""}`}>
				<div className="text-center">
					<img src="/image 45.png" alt="logo" className="img-fluid side_logo_height_width mt-3" />
				</div>
					...
					{navItems.map((item, index) => (
						<Link
							...
							className={`nav-item d-flex flex-column align-items-center my-2 text-decoration-none ${
								location.pathname === item.path ? "active py-2 px-4 rounded-3" : ""
							}`}
						>
							<img
								src={`${item.image}`}
							...
							/>
							<span className={`mt-1 small ${location.pathname === item.path ? "text_nav_item" : "text-muted"}`}>
								{item.name}
							</span>
						</Link>
					))}
